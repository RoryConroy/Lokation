...
fun uploadImageView(app: LokationApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Lokation", "uploadTask.exception" + error)
        }
    }
...
fun updateAllLokations(app: LokationApp) {
    val userId = app.currentUser.uid
    val userEmail = app.currentUser.email
    val lokationRef = app.database.ref.child("lokations")
                                  .orderByChild("email")
    val userlokationRef = app.database.ref.child("user-lokations")
                                  .child(userId).orderByChild("uid")

    lokationRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    userlokationRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
        override fun onCancelled(error: DatabaseError) {}
        override fun onDataChange(snapshot: DataSnapshot) {
            snapshot.children.forEach {
...
fun writeImageRef(app: LokationApp, imageRef: String) {
    val userId = app.currentUser.uid
...
fun validatePhoto(app: LokationApp, activity: Activity) {
...
fun checkExistingPhoto(app: LokationApp, activity: Activity) {
