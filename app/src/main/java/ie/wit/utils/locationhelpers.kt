...
fun setCurrentLocation(app: LokationApp) {
...
@SuppressLint("MissingPermission")
fun trackLocation(app: LokationApp) {
...
fun setMapMarker(app: LokationApp) {
...
fun getAllLokations(app: LokationApp) {
    val lokationsList = ArrayList<LokationModel>()

    app.database.child("user-lokations").child(app.currentUser.uid)
        .addValueEventListener(object : ValueEventListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                children.forEach {
                    val lokation = it.
                        getValue<LokationModel>(LokationModel::class.java)
                    lokationsList.add(lokation!!)
                }
                addMapMarkers(lokationsList, app.mMap)
            }
        })

}

fun getFavouriteLokations(app: LokationApp) {
    val lokationsList = ArrayList<LokationModel>()
...
fun addMapMarkers(dl : ArrayList<LokationModel>, map: GoogleMap) {
    dl.forEach {
        map.addMarker(
            MarkerOptions().position(LatLng(it.latitude, it.longitude))
                .title("Title: ${it.Title}, Rating: ${it.Rating},Short Description ${it.Short} ")
                .icon(
